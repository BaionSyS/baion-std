/* BAION SHA-256 for C — vendored FIPS 180-4 implementation.
 *
...
static void sha256_transform(baion_sha256_ctx* ctx, const uint8_t data[64])
{
...
/* CROSS-LINEAGE CONTRACT: identical input bytes must produce identical
 * 32-byte output across all seven STDs (vendored FIPS 180-4 in each).
 * Output feeds the SHA-256 storage keys —
 * divergence breaks every cross-lineage storage check. */
void baion_sha256(const uint8_t* data, size_t len, uint8_t hash[32])
{
    baion_sha256_ctx ctx;
    baion_sha256_init(&ctx);
    baion_sha256_update(&ctx, data, len);
    baion_sha256_final(&ctx, hash);
}
