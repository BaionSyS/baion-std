...
main = do
  ...
  -- Raw-control-byte gate runs BEFORE the decode: it is purely
  -- byte-level (safe on malformed input), and aeson's current lexer
  -- would otherwise reject the same inputs first with a parser-internal
  -- message — running the contract check first keeps the error text
  -- cross-lineage uniform and keeps the contract off a dependency's
  -- strictness (an aeson upgrade cannot silently relax it).
  case checkControlBytes input of
    Left err -> do
